...
reviews = pd.read_csv("/home/nwang/proj/data/intermediate/SF_one_star_reviews.csv")
...
#for i in range(0, len(reviews)):
#    paragraphs = reviews.loc[i]["review main text"].split("\n")
#    for paragraph in paragraphs:
#        if paragraph != "" and len(paragraph) > 300:
#            review_main_text_list.append(paragraph)
...
# Check topics.
topic_list = ldamodel.print_topics(num_topics=30, num_words=15)
for i in topic_list:
    str1 = str(i[1])
    for c in "0123456789+*\".":
        str1 = str1.replace(c, "")
